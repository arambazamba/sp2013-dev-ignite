...
namespace TaxonomySSOM
{
    class Program
    {
        ...
        {
            using (SPSite siteCollection = new SPSite("http://intranet.contoso.com/sites/ecm"))
            {
                TaxonomySession taxSession = new TaxonomySession(siteCollection);
                TermStore termStore = taxSession.TermStores[0];
                Group termGroup = termStore.Groups.Single(tg => tg.Name == "Sample Taxonomy");
                TermSet termSet = termGroup.TermSets.Single(ts => ts.Name == "Locations");
